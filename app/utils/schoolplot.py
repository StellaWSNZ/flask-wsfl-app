...
DEBUG = False
DB = True
MOENUMBER = 1522  # ✅ Set your actual MOE number here
TERM = 2
CALENDARYEAR = 2025
OUTPUT_FILENAME = f"SchoolResultReport_{TERM}_{CALENDARYEAR}.pdf"
...
def load_national_results(con, calendaryear, term, moenumber, from_db=True):
    if from_db:
        with con.connect() as connection:
            result = connection.execute(
                text("EXEC GetSchoolNationalRatesSmart :CalendarYear, :Term, :MOENumber"),
                {"CalendarYear": calendaryear, "Term": term, "MOENumber": moenumber}
            )
...
def create_school_report(calendaryear, term, moenumber, title=None):
    con = get_db_engine()
    print(f"GetSchoolNationalRatesSmart for MOENumber={moenumber}, Term={term}, Year={calendaryear}")

    df = load_national_results(con, calendaryear, term, moenumber, from_db=True)

    if df.empty:
        print("⚠️ No data returned from stored procedure.")
        return None

    expected_columns = ['CompetencyDesc', 'YearGroupDesc']
    ...
    if not title:
        title = f'National Rate (LY) vs School Rate (YTD) | Term {term}, {calendaryear}'

    fig = make_figure(df, DEBUG=False, TITLE=title, PAGE_SIZE=PAGE_SIZE,
                      TITLE_SPACE=TITLE_SPACE, subtitle_space=SUBTITLE_SPACE,
                      row_heights=row_heights, BUFFER=BUFFER)
    return fig
...
if __name__ == "__main__":
    con = get_db_engine()
    df = load_national_results(con, CALENDARYEAR, TERM, MOENUMBER, DB)
